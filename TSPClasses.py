...
###################################################################################################
# Take in a cost of route, paths, state, route so far, length of route and last city arrived at


class TSPNode:
    def __init__(self, cost, paths, route, length, last):
        self.cost = cost
        self.paths = paths
        self.route = route
        self.length = length
    ...
    # Used for comparision in the heapq, cost of the solution plus the cost over the number of cities visited
    # Done to give slight priority to solutions that are further along
    # Time Complexity O(1)
    def __lt__(self, other):
        return self.cost/self.length - math.pow(self.length, 2) < other.cost/other.length - math.pow(other.length, 2)
